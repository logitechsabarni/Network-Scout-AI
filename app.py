...
def check_threat(ip):
    try:
        ...
        return {
            ...
            "mobile":  j.get("mobile", False),
            "isp":     j.get("isp", ""),
        }
    except: return {}

def get_cert_info(hostname):
    """Fetch SSL/TLS certificate info"""
    import ssl
    try:
    ...
    if os_n == "Linux":
        try:
            ...
            for line in out.strip().splitlines():
                ...
                if len(p) >= 4:
                    ...
                    seen.add(key)
                    nets.append({"ssid": ssid, "signal": int(p[1]) if p[1].isdigit() else 0,
                                 "security": p[2] or "OPEN", "bssid": p[3],
        ...
        try:
            ...
            for line in out.strip().splitlines()[1:]:
                p = line.split()
                if len(p) >= 3:
                    nets.append({"ssid": p[0], "signal": abs(int(p[2])) if p[2].lstrip('-').isdigit() else 0,
                                 "security": p[-1] if len(p) > 5 else "OPEN", "bssid": p[1],
        ...
        if "error" in d:
            ...
            if threat and (threat.get("proxy") or threat.get("hosting")):
                ...
                <div class="anomaly-alert">
                  ...
                  <div class="anomaly-body">
                    ...
                    <div class="anomaly-detail">This IP is flagged as a {proxy_type.lower()} endpoint.
                    Traffic may be anonymized. ISP: {threat.get("isp","—")}</div>
                  </div>
                </div>""", unsafe_allow_html=True)
            ...
            with cl:
                ...
                if d:
                    ...
                    st.download_button(
                        label="⬇ Export Location Data (JSON)",
                        data=export_json,
                        file_name=f"netpulse_ip_{d.get('ip','unknown')}.json",
            ...
            with cr:
                ...
                if threat:
                    ...
                    items = [
                        ("🔒", "Proxy / VPN",     threat.get("proxy", False)),
...
with tab2:
    ...
    req_map = {"Linux": "nmcli (network-manager)", "Windows": "netsh (built-in)", "Darwin": "airport (built-in)"}
    st.markdown(f'<div class="info-box info-box-blue">📡 Platform: <strong>{sys_n}</strong> · Tool: {req_map.get(sys_n,"unknown")} · Forces rescan before listing.</div>', unsafe_allow_html=True)
    ...
    if st.button("📡 Scan WiFi Networks", use_container_width=False):
        with st.spinner("Scanning wireless networks..."):
            st.session_state["wifi_data"] = scan_wifi()

    nets = st.session_state.get("wifi_data")
    if nets is not None:
        filtered = nets
        ...
        for net in filtered:
            ...
            freq   = net.get("freq", "?")
            band   = "5 GHz" if "5" in str(freq) else "2.4 GHz" if "2.4" in str(freq) or int(net.get("channel","0") or 0) <= 14 else "?"
        ...
        if nets:
            ...
            st.download_button(
                label="⬇ Export WiFi Scan (JSON)",
                data=wifi_json,
                file_name=f"netpulse_wifi_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
            )
...
with tab4:
    ...
    with pt1:
        ...
        if pres:
            ...
            st.download_button(
                label="⬇ Export Scan Results (JSON)",
                data=scan_export,
                file_name=f"netpulse_portscan_{ph_}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
            )
            ...
            if err:
                ...
                for r in range_results:
                    if r["open"]:
                        ...
                        range_rows += (
                            f'<tr><td style="color:var(--blue2);font-weight:600">{r["ip"]}</td>'
                ...
                if range_results:
                    range_export = export_to_json({
                        ...
                        "ports_scanned": r_ports,
                        "scan_time": datetime.now().isoformat(),
                        "results": range_results
                    })
                    st.download_button(
                        label="⬇ Export Range Scan (JSON)",
                        data=range_export,
                        file_name=f"netpulse_rangescan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                        mime="application/json",
                    )
...
with tab5:
    ...
    if ifaces:
        ...
        for i in ifaces:
            ...
            iface_rows += (
                f'<tr>'
                f'<td style="color:var(--bright);font-weight:600">{i["name"]}</td>'
                f'<td>{up_badge}</td>'
                ...
                f'<td style="color:var(--dim)">{i["mtu"]}</td>'
                f'</tr>'
            )
    ...
    st.markdown('<div class="sec"><span class="sec-icon">📊</span><span class="sec-txt">I/O Throughput (1-second sample)</span><div class="sec-line"></div></div>', unsafe_allow_html=True)
    if st.button("⚡ Sample I/O"):
        with st.spinner("Sampling 1 second..."):
            st.session_state["net_stats"] = net_io()
    ...
    if stats and "error" not in stats:
        ...
        for col, (v, l, c) in zip([c1, c2, c3, c4], [
            ...
            with col:
                st.markdown(f'<div class="tile"><div class="tile-val" style="color:{c};font-size:1.2rem">{v}</div><div class="tile-lbl">{l}</div></div>', unsafe_allow_html=True)
    ...
    if st.button("🔗 List Active Connections"):
        try:
            ...
            if conns:
                ...
                for c in conns[:30]:
                    ...
                    conn_rows += (
                        f'<tr>'
                        f'<td style="color:var(--blue2)">{c.laddr.ip}:{c.laddr.port}</td>'
                        f'<td style="color:var(--text2)">{raddr_ip}:{raddr_port}</td>'
                        f'<td>{ext_badge}</td>'
                        f'<td><span class="badge b-green">{c.status}</span></td>'
                        f'<td style="color:var(--dim)">{pid_str}</td>'
                        f'</tr>'
                    )
                st.markdown(f'<div class="wifi-wrap"><table class="wtable"><thead><tr><th>Local</th><th>Remote</th><th>Type</th><th>Status</th><th>PID</th></tr></thead><tbody>{conn_rows}</tbody></table></div>', unsafe_allow_html=True)
            else:
                st.info("No established connections found (may need elevated privileges).")
        except Exception as e:
            st.warning(f"Could not list connections: {e}")
        ...
        if run_s and ci_n:
            ...
            if "error" in sub:
                ...
                st.download_button(
                    ...
                    data=sub_export,
                    file_name=f"netpulse_subnet_{ci_n.replace('/','-')}.json",
                    mime="application/json",
                )
    ...
    with tt3:
        ...
        if mp_results:
            ...
            if valid_results:
                ...
                st.markdown(f"""
                <div class="sparkline-wrap">
                  <div class="sparkline-title">📊 Latency Comparison</div>
                  {hist_bars}
                </div>""", unsafe_allow_html=True)
            ...
            for h, avg in mp_results:
                ...
                mp_rows += (
                    f'<tr>'
                    f'<td style="color:var(--bright)">{h}</td>'
                    f'<td><span class="badge {bc}">{bl}</span></td>'
                    f'<td style="color:var(--dim)">{lat_str} ms</td>'
                    f'</tr>'
                )
            ...
            st.download_button(
                label="⬇ Export Multi-Ping Results (JSON)",
                data=ping_export,
                file_name=f"netpulse_multiping_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                mime="application/json",
            )
        ...
        if hr4:
            if hr4.get("error"):
                ...
                st.download_button(
                    ...
                    data=host_export,
                    file_name=f"netpulse_hostname_{h4_name}.json",
                    mime="application/json",
                )
        ...
        if cert_data:
            ...
            if "error" in ci_:
                ...
                if days_left is not None and days_left <= 30:
                    ...
                    <div class="anomaly-alert">
                      ...
                      <div class="anomaly-body">
                        ...
                        <div class="anomaly-detail">Cert for {ch_} expires: {ci_["not_after"]}. Renew immediately to avoid downtime.</div>
                      </div>
                    </div>""", unsafe_allow_html=True)
                ...
                st.download_button(
                    ...
                    data=cert_export,
                    file_name=f"netpulse_cert_{ch_}.json",
                    mime="application/json",
                )
        ...
        if wr:
            if "error" in wr:
                ...
                st.download_button(
                    ...
                    data=whois_export,
                    file_name=f"netpulse_whois_{wr.get('query','unknown')}.json",
                    mime="application/json",
                )
